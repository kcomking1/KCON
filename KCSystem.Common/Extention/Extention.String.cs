...
namespace KCSystem.Common
{
    public static partial class Extention
    {
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static DateTime? ToNullabelDateTime(this string str)
        {
            if (string.IsNullOrEmpty(str))
            {
                return null;
        ...
        public static T ToEnum<T>(this string enumText)
        {
            ...
            {
                return typeof(T).GetEnumValues().CastToList<T>().FirstOrDefault();
            }
            if (!typeof(T).GetEnumNames().CastToList<string>().Contains(enumText))
            {
                var descriptionAttribute = typeof(T).GetCustomAttributes<DescriptionAttribute>().FirstOrDefault();
                var name = descriptionAttribute != null ? descriptionAttribute.Description : typeof(T).Name;
                throw new Exception($"{name}中不包含[{enumText}]");
            }
            return typeof(T).GetEnumValues().CastToList<T>().FirstOrDefault(x => x.ToString() == enumText);  
        }
        ...
        /// <returns></returns>
        public static decimal CalcByDataTable(this string expression)
        {
